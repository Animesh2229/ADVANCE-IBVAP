...
app = FastAPI(title="IBVAP Central API", version="1.3.1", docs_url="/docs" if ENVIRONMENT == "development" else None)
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_db), request: Request = None):
    ...
    if len(_login_attempts.get(client_ip, [])) >= 10:
...
async def receive_secure_alert(
    ...
    sig = x_ibvap_signature or alert_data.get("signature")
    ts_hdr = x_ibvap_timestamp or alert_data.get("timestamp")
    if is_replay is not None and is_replay(str(ts_hdr or ""), str(sig or "")):
        raise HTTPException(status_code=409, detail="Replay detected: alert already accepted")
...
async def list_face_watchlist(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    return [{"id": r.id, "person_name": r.person_name, "camera_id": r.camera_id, "timestamp": r.timestamp} for r in rows]


@app.post("/api/v1/face-watchlist")
async def add_face_watchlist(data: dict, current_user: User = Depends(require_roles(["admin", "commander"])), db: AsyncSession = Depends(get_db)):
    emb = data.get("embedding")
    if not emb or not data.get("person_name"):
        raise HTTPException(status_code=400, detail="person_name and embedding required")
    row = FaceEmbedding(person_name=data["person_name"], embedding=emb, camera_id=data.get("camera_id") or "enroll", is_watchlist=True)
    db.add(row)
    await db.commit()
...
async def c2_export(data: dict = None, current_user: User = Depends(require_roles(["admin", "commander"])), db: AsyncSession = Depends(get_db)):
    data = data or {}
    limit = min(int(data.get("limit", 50)), 200)
    result = await db.execute(select(Alert).where(Alert.priority.in_(["HIGH", "MEDIUM"])).order_by(Alert.timestamp.desc()).limit(limit))
    ...
    return {
        ...
        "events": [{
            "event_id": a.id, "camera_id": a.camera_id, "type": a.alert_type, "subtype": a.subtype,
            "priority": a.priority, "status": a.status, "confidence": a.confidence,
            "timestamp": a.timestamp.isoformat() if a.timestamp else None, "event_hash": a.event_hash,
        } for a in alerts],
    }
...
async def websocket_alerts(websocket: WebSocket, token: Optional[str] = None):
    """Require JWT via ?token=... query param or access_token cookie."""
    if ws_manager is None:
        await websocket.close(code=1011)
        return
    q_token = token or websocket.query_params.get("token")
    c_token = websocket.cookies.get("access_token")
    jwt_token = q_token or c_token
    if not jwt_token:
...
@app.get("/")
async def root():
    return {"message": "IBVAP Central API is running", "version": "1.3.1"}
